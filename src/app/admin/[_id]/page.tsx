...
function UpdateAdmin() {
  ...
  const fetchAdminData = useCallback(async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BaseUrl}/admin/list/${_id}`,
        {
          method: "GET",
        },
      );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const submissionData = {
      ...
      password: formData.password || existingPassword,
    };

    try {
      const response = await fetch(
        // `http://localhost:8000/api/admin/update/${_id}`,
        `${process.env.NEXT_PUBLIC_BaseUrl}/admin/update/${_id}`,
        {
          method: "PUT",
          headers: {
          ...
          <form onSubmit={handleSubmit}>
            <div className="p-6.5">
              ...
              <div className="mb-2">
                <input
                  type="submit"
                  value={loading ? "Processing..." : "Update Account"}
